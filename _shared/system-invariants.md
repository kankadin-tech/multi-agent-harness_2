...
| INV9 | gemini 백엔드가 `_shared/backends.json`에서 `agy` CLI(call_type cli·command agy)이고, **backends.json의 모델 핀이 routing.md·task-folder.md의 사본과 일치**하며 폐기 세대(`gemini-3.1-pro-high`)가 backends에 활성값으로 없음 | 정본이 폐기 프록시/known-bad 경로 호출, 또는 핀↔문서 드리프트 (D4 위반) |
...
| INV15 | 승인 게이트가 코드로 강제됨: `_shared/hooks/approval_gate.py`가 `workers_approved`를 판정하고, `_shared/adapters/call_worker.sh`가 그 판정기를 호출 | D14 위반 — 게이트가 문서 규약으로 되돌아가 무승인 호출이 통과 |
