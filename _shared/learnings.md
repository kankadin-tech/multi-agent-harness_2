...
## [2026-08-01] 문서로만 존재하는 안전장치는 안전장치가 아니다 (harness-integrity-fixes)

- **증상**: 감사 결과 이 하네스의 핵심 안전장치인 승인 게이트에 **기술적 강제가 0**이었다. `call_worker.sh`는 `task.md`를 읽지도 않았고, `check-invariants.sh`는 `max_worker_calls`라는 *문자열이 문서에 적혀 있는지*만 봤다. 셸에서 디스패처를 직접 치면 무승인·무예산 호출이 그대로 나갔다.
- **일반 교훈 ①**: 규칙을 문서에 쓰고 "그 문자열이 문서에 있는지"를 검사하면, **검사가 통과해도 규칙은 집행되지 않는다.** 불변식은 *문서에 규칙이 적혀 있음*이 아니라 *규칙을 집행하는 코드가 호출됨*을 검사해야 한다. (INV15/INV14가 판정기 존재 + 디스패처 호출부 양쪽을 보는 이유.)
- **일반 교훈 ②** — 불변식에 값을 굽지 말 것: INV9가 `gemini-3.1-pro-high`를 하드코딩해, 핀을 정상 갱신했더니 자가점검이 FAIL로 뒤집혔고 그 상태로 릴리스됐다. 값이 아니라 **관계**(데이터 정본 ↔ 문서 사본 일치)를 검사하면 세대 교체에 썩지 않는다.
- **일반 교훈 ③** — 규칙의 권위와 사실의 권위를 분리하라: 실행 파라미터(모델 핀·sandbox)의 정본은 데이터 파일이고 문서 서술은 사본이다. 이걸 구분하지 않아 하위 문서(routing.md)가 상위 결정문서(design-basis D4)를 "무효"라고 선언하는 역전이 생겼다.
- **일반 교훈 ④** — `die`를 명령치환 안에서 쓰지 말 것: 서브셸만 죽고 호출부엔 빈 문자열이 남는다. 이 하네스에선 그게 "폴백이 있으면 조용히 다른 백엔드 실행, 없으면 jq 크래시"로 나타났다. 실패 경로도 **항상 유효한 반환값(envelope)** 을 내도록 설계하라.
- **일반 교훈 ⑤** — 생성기가 관리하는 파일에 손으로 줄을 추가하지 말 것: 재설치가 지운다(실제 발생). 관례 경로를 **템플릿 정본**에 넣으면 재생성에도 살아남고, 대상 파일이 없는 설치에선 무해하다.
- **일반 교훈 ⑥** — 누적물과 배포물을 구분하라: `learnings.md`처럼 번들이 *씨앗*을 주고 사용자가 append하는 파일은 update가 덮으면 안 된다(KI-4). 3-way 병합 같은 기교보다 **scaffold-once**가 의미에 정확하고 실패 모드도 적다.
**worker**: orchestrator (워커 호출 0 — 안전 크리티컬 스크립트의 외과수술식 수정이라 직접 수행)
