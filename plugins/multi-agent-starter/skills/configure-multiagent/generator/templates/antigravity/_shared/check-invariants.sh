...
# INV2 — gemini 워커/gemini-critic 활성 참조 없어야 PASS (오케스트레이터가 Gemini)
if grep -rn 'call_worker.sh gemini\|gemini-critic\|- \*\*gemini\*\*' \
     "$ROOT/AGENTS.md" "$ROOT/README.md" "$ROOT/_shared/routing.md" \
     "$ROOT/_shared/approval-policy.md" "$ROOT/_templates" >/dev/null 2>&1; then
  fail "INV2 gemini 워커/gemini-critic 활성 참조 존재"
else
  pass "INV2 gemini 워커/gemini-critic 활성 참조 없음"
fi

# INV3 — codex-critic 비평 워커 존재
if grep -rn 'codex-critic' "$ROOT/AGENTS.md" "$ROOT/_shared/routing.md" "$ROOT/_templates" >/dev/null 2>&1; then
  pass "INV3 codex-critic 존재"
else
  fail "INV3 codex-critic 없음"
fi
...
# INV9 — 오케스트레이터 Gemini 3.1 Pro High + 워커셋 claude-main/codex-main/codex-critic
inv9_ok=1
grep -q 'Gemini 3.1 Pro High' "$ROOT/AGENTS.md" || inv9_ok=0
for w in claude-main codex-main codex-critic; do
  grep -q "\"$w\"" "$ROOT/_shared/backends.json" 2>/dev/null || inv9_ok=0
done
[ "$inv9_ok" = 1 ] && pass "INV9 오케스트레이터·워커셋 정합" \
                   || fail "INV9 오케스트레이터·워커셋 불일치"

# INV10 — gemini 워커 호출/옛 프록시 활성 없어야 PASS (폐기문맥 제외)
if grep -rn 'call_worker.sh gemini\|mcp__gemini-pro__\|mcp__gemini__gemini_' \
     "$ROOT/_shared/routing.md" "$ROOT/_templates/task-folder.md" "$ROOT/AGENTS.md" "$ROOT/_shared/backends.json" 2>/dev/null \
     | grep -viE '폐기|deprecat' | grep -q .; then
  fail "INV10 gemini 워커 호출/옛 프록시 활성 존재"
else
  pass "INV10 gemini 워커 호출/옛 프록시 활성 없음"
fi
