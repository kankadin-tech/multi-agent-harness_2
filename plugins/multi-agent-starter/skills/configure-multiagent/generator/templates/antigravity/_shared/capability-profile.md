...
- **2026-07-13** 초기 배정 + computer-use 슬롯 신설. 근거: 외부 리뷰 10건 종합 판정
  ...
  대규모 구현·테스트·브라우저 조작·비용·속도 = GPT 우위로 수렴. 요지는 design-basis **D10** (종전 이 줄이 "D8"로 잘못 인용했다 — 2026-08-01 정정).

- **2026-08-01** 배정 변경 없음, 티어 점검만 수행. 2026-07-13 판정은 그 시점의 플래그십 비교이며
  이후 세대 교체가 반영되지 않았다. **배정을 바꾸려면 새 판정 자료가 필요하다** — 자료 없이 인상만으로
  슬롯을 옮기지 말 것(이 파일의 존재 이유).
